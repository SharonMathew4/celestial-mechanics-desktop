...
public class RenderLoop : IDisposable
{
    ...
    /// <summary>
    /// Raised when render loop initialization or frame rendering fails.
    /// </summary>
    ...
    private void Loop()
    {
        try
        {
            ...
            if (!Win32Interop.wglMakeCurrent(host.Hdc, host.Hglrc))
                throw new InvalidOperationException("Failed to bind WGL context to render thread.");
