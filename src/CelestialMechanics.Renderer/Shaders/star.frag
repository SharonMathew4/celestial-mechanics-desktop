...
vec3 shadeExplosion(vec3 norm, vec3 localN, vec3 viewDir, float luminosity, float ageProgress, float tier) {
    float edge = 1.0 - abs(dot(norm, viewDir));
    
    float coreSize = smoothstep(1.0, 0.0, ageProgress) * 0.8;
    float coreMask = smoothstep(coreSize + 0.2, coreSize, length(localN));
    
    float expansion = ageProgress; // 0 (start) -> 1 (end)
    float ringRadius = expansion * 1.5;
    float ringThickness = mix(0.1, 0.02, expansion);
    float ring = exp(-pow((length(localN.xz) - ringRadius) / ringThickness, 2.0));
    ring *= (1.0 - abs(localN.y) * 2.0); 
    
    float debris = fbm(localN * mix(10.0, 25.0, expansion) + vec3(uTime * 0.3));
    float debrisHole = smoothstep(0.2, 0.6, debris);
    float debrisMask = debrisHole * edge * (1.0 - ageProgress); 
    
    vec3 coreColor = vec3(10.0, 9.5, 8.5);
    vec3 ringColor = vec3(8.0, 5.0, 2.0);
    vec3 debrisColor = vec3(4.0, 2.0, 0.8);
    
    if (tier >= 3.5) { 
        coreColor = vec3(20.0, 18.0, 15.0);
        ringColor = vec3(15.0, 10.0, 8.0);
        debrisColor = vec3(8.0, 2.0, 1.0);
        ring *= 2.0; 
    } else if (tier >= 2.5) {
        coreColor = vec3(6.0, 9.0, 18.0); 
        ringColor = vec3(2.0, 5.0, 15.0);
        debrisColor = vec3(0.5, 2.0, 8.0);
        ringRadius = expansion * 1.8; 
    } else if (tier >= 1.5) {
        coreColor = vec3(14.0, 10.0, 8.0);
        ringColor = vec3(12.0, 4.0, 1.0); 
        debrisColor = vec3(6.0, 1.0, 0.5);
    } else {
        coreColor = vec3(8.0, 8.0, 7.0);
        ringColor = vec3(4.0, 3.0, 2.0);
        debrisColor = vec3(2.0, 1.0, 0.5);
        ring *= 0.5;
    }
    
    vec3 result = coreColor * coreMask + ringColor * ring * 0.8 + debrisColor * debrisMask * 0.5;
    result *= max(luminosity, 1.0);
    
    vec3 rimColor = mix(vec3(1.0, 0.85, 0.65), coreColor * 0.2, expansion);
    result += rimColor * pow(edge, 1.2) * luminosity * 2.0 * (1.0 - expansion);
...
void main() {
    ...
    if(visualType >= 9.5) {
        float ageProgress = vVisual.z;
        float tier = vVisual.w;
        vec3 explosionColor = shadeExplosion(norm, localN, viewDir, luminosity, ageProgress, tier);
